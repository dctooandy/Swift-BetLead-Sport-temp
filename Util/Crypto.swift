...
extension String {
    ...
    func md5(output: HashOutputType = .hex) -> String? {
        return hashed(.md5, output: output)
    }
    
    
    // hash string
    func hashed(_ type: HashType, output: HashOutputType = .hex) -> String? {
        guard let str = data(using: .utf8) else { return nil }
        return str.hashed(type, output: output)
    }
}
